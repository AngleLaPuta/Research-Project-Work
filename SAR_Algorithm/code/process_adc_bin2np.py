...
parser.add_argument('--raw_data', metavar='DIR', default='/localscratch/maolin/projects/Pitt-Radar/dataset/radar_dataset_bin/',
                    help='path to data folder')
parser.add_argument('--train_output', '-o1', metavar='DIR', default='/localscratch/maolin/projects/Pitt-Radar/dataset/radar_numpy_train/',
                    help='path to output folder. If not set, will be created in data folder')
parser.add_argument('--test_output', '-o2', metavar='DIR', default='/localscratch/maolin/projects/Pitt-Radar/dataset/radar_numpy_test/',
                    help='path to output folder. If not set, will be created in data folder')

args = parser.parse_args()


if not os.path.exists(args.train_output) and not os.path.exists(args.test_output):
...
def RAD_map(range_plot):
    ...
    for i in range(64, 67):
        axs[i-64].imshow(out_img[:, :, i], cmap='viridis')
        axs[i-64].set_title(f'Slice at Azimuth index {i}')
        axs[i-64].axis('off')
...
for i in range(len(files)):
    ...
    # output = RAD_map(file_adc)

    if files[i] in train_files:
        np.save(args.train_output+'{:d}'.format(i),file_adc)
        print('Train: ',i)
    else:
        np.save(args.test_output+'{:d}'.format(i),file_adc)
        print('Test: ',i)
