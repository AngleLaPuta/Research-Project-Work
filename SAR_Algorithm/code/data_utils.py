...
class TrainDatasetFromFolder_SAR_adc(Dataset):
    def __init__(self, adc_data_dir, num_low_receiver, hr_data_dir, lr_data_dir, crop_size, upscale_factor, index_list=None, num_high_receiver=12):
        super().__init__()
        if index_list != None: #Use this function for now
            self.adc_filenames = [os.path.join(root, filename) for root, dirs, files in os.walk(adc_data_dir) for filename in files if filename[-4:] == '.mat']
            # self.hr_filenames = [x for x in sorted(glob(hr_data_dir)) if is_numpy_file(x)]
            # self.lr_filenames = [x for x in sorted(glob(lr_data_dir)) if is_numpy_file(x)]
            print(len(self.adc_filenames))
            # print(len(self.lr_filenames))
    ...
    def __getitem__(self, index):
        ...
        data = scipy.io.loadmat(self.adc_filenames[index])
        numpy_adc = data['rawDataUniform']
        numpy_adc = numpy_adc.transpose(1, 0, 2)
...
class ValDatasetFromFolder_SAR_adc(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = scipy.io.loadmat(self.adc_filenames[index])
        filename = self.adc_filenames[index]

        # data = scipy.io.loadmat('/localscratch/maolin/projects/Pitt-Radar/dataset/leaf_wetness_train/0_0716_150_1/0_0716_150_1_150.mat')
        # filename = '/localscratch/maolin/projects/Pitt-Radar/dataset/leaf_wetness_train/0_0716_150_1/0_0716_150_1_150.mat'
        ...
        return adc_data_high, adc_data_low, 2, 2, filename
        return adc_data_high, adc_data_low, lr_image[None], hr_image[None]
        return adc_data, adc_data_low, lr_image[None], hr_image[None]
        return (lr_image[None]).type(torch.FloatTensor), (lr_image[None]).type(torch.FloatTensor), (hr_image[None]).type(torch.FloatTensor)
...
class TrainDatasetFromFolder_SAR3D_image(Dataset):
    def __init__(self, adc_data_dir, index_list=None):
        super().__init__()
        if index_list != None: #Use this function for now
            self.hr_image_filenames = [os.path.join(root, filename) for root, dirs, files in os.walk(adc_data_dir) for filename in files if filename.endswith('.jpg') and '_lr' not in filename]
            self.lr_image_filenames = [os.path.join(root, filename) for root, dirs, files in os.walk(adc_data_dir) for filename in files if filename[-7:] == '_lr.jpg']
            
            print("Number of Training HR images: ")
            print(len(self.hr_image_filenames))
            print("Number of Training LR images: ")
            print(len(self.lr_image_filenames))
    ...
    def __getitem__(self, index):
        ...
        hr_image_tensor = self.transform(hr_image)

        hr_image_filename = self.hr_image_filenames[index]
        lr_image_filename = hr_image_filename.replace('.jpg', '_lr.jpg')
        lr_image = Image.open(lr_image_filename)
        lr_image_tensor = self.transform(lr_image)

        return hr_image_tensor, lr_image_tensor
    
    def __len__(self):
        return len(self.hr_image_filenames)

class ValDatasetFromFolder_SAR3D_image(Dataset):
    def __init__(self, adc_data_dir, index_list=None):
        super().__init__()
        if index_list != None: #Use this function for now
            self.hr_image_filenames = [os.path.join(root, filename) for root, dirs, files in os.walk(adc_data_dir) for filename in files if filename.endswith('.jpg') and '_lr' not in filename]
            ...
            print("Number of Test HR images: ")
            print(len(self.hr_image_filenames))
            print("Number of Test LR images: ")
            print(len(self.lr_image_filenames))
    ...
    def __getitem__(self, index):
        ...
        return hr_image_tensor, lr_image_tensor, hr_image_filename
