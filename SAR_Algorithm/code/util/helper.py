...
def complexTo2Channels(target_array):
    ...
    # This function only keeps magnitude
...
def getLog(target_array, scalar=1., log_10=True):
    """ get Log values """
    if log_10:
        return scalar * np.log10(target_array)
...
def norm2Image_abs(array):
    ...
    # pdb.set_trace()
    array = (array-90)/50
    img = plt.cm.viridis((array))
    img *= 255.
    img = img.astype(np.uint8)
    return img

def norm2Image_abs2(array):
    """ normalize to image format (uint8) """
    # norm_sig = plt.Normalize()
    # img = plt.cm.viridis(norm_sig(array))
    # import pdb
...
def convert_array(val):
    if isinstance(val, np.ndarray):
        if len(val) > 1:
            return val.tolist()  # to list
        else:
            return val.item()  # to scalar
    return val
